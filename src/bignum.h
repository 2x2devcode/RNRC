...
//
// Updated for OpenSSL 1.1+/3.x (opaque BIGNUM) — Ubuntu 22.04 / 24.04 / 26.04
...
/** C++ wrapper for BIGNUM (OpenSSL bignum) — compatible with opaque BIGNUM */
class CBigNum
{
protected:
    BIGNUM* bn;
    ...
    void alloc()
    {
        bn = BN_new();
        if (bn == NULL)
            throw bignum_error("CBigNum::alloc : BN_new failed");
    }
...
public:
    ...
    CBigNum(signed char n)        { alloc(); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(short n)              { alloc(); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(int n)                { alloc(); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(long n)               { alloc(); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(long long n)          { alloc(); setint64(n); }
    CBigNum(unsigned char n)      { alloc(); setulong(n); }
    CBigNum(unsigned short n)     { alloc(); setulong(n); }
    CBigNum(unsigned int n)       { alloc(); setulong(n); }
    CBigNum(unsigned long n)      { alloc(); setulong(n); }
    CBigNum(unsigned long long n) { alloc(); setuint64(n); }
    explicit CBigNum(uint256 n)   { alloc(); setuint256(n); }
